...
app.put('/musica/:id', (req, res) => {
    ...
    const id = req.params.id
       const data = async() =>{
           try{
               const musicaid = await music.listar_musicas_ID(id)
               const musicaAlterada = new musica(body.nome || musicaid[0].nome,
                                                body.genero || musicaid[0].genero,
                                                body.tempo_de_duracao || musicaid[0].tempo_de_duracao)
               const musicas = await music.altera_musicas(id, musicaAlterada)
